      ...
      <div className="max-w-[960px] mx-auto">
        <div className="flex flex-col gap-12">
          <div className="text-left">
            <h2 className="text-4xl font-bold text-[#121716] mb-4">Our Values</h2>
            <p className="text-lg text-[#68827c] max-w-2xl">
          ...
          <div className="grid grid-cols-3 gap-6">
            {values.map((item, index) => (
              <div key={index} className="group p-8 rounded-2xl bg-white border border-[#f0f0f0] hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
                <div className="size-12 rounded-full bg-[#217864]/10 flex items-center justify-center text-[#217864] mb-6 group-hover:bg-[#217864] group-hover:text-white transition-colors">
                  <span className="material-symbols-outlined">{item.icon}</span>
                </div>
                <h3 className="text-xl font-bold text-[#121716] mb-2">{item.title}</h3>
                <p className="text-[#68827c] text-sm leading-relaxed">
                  {item.desc}
                </p>
              </div>
            ))}
